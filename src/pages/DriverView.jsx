...
const DriverView = () => {
  ...
  function calculateTripMetrics(openDateTime, closeDateTime) {
    ...
    // Prevent negative KM calculation
    if (data.closeKm <= effectiveOpenKm) {
      console.log(" this is the values of open km" ,effectiveOpenKm,"and close", data.closeKm);
      
      toast.warn("Invalid KM: Closing KM must be greater than Opening KM.")
      console.warn("Invalid KM: Closing KM must be greater than Opening KM.");
      return null;
    }
    ...
    if (diffMs < 0) {
      toast.warn("Invalid KM: Closing Hr must be greater than Opening Hr.")
      console.warn("Invalid time: Closing time must be after opening time." ,diffMs);
      return null;
    }
  ...
  const validateForm = () => {
    ...
    const openKmValue = openKm ?? tripDetails?.openKm;
    if (data.closeKm <= openKmValue) {
    ...
    if (isNaN(openDateTime.getTime()) || isNaN(closeDateTime.getTime())) {
      toast.warning("Invalid date/time format.");
      return false;
    }
    ...
    if (result.totalHr < 0) {
      toast.warning("Total hours must be a non-negative value.");
      return false;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log(" this is the data been sending ", formData);
