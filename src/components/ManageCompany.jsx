...
const ManageCompany = () => {
  ...
  const [customerData, setCustomerData] = useState({ companyName: "", phoneNo: "" });
  ...
  const handleDeleteCustomer = useCallback(async (customerId) => {
    if (window.confirm("Are you sure you want to delete this customer?")) {
      try {
        const response = await LocalClient.delete(`${API.deleteCustomer}/${customerId}`);
        if (response.status === 200) {
  ...
  // const handleEditCompany = useCallback((companyId) => {
  //   console.log(`Editing company with ID: ${companyId}`);
  //   setMenuOpen(null);
  // }, []);

  const toggleMenu = useCallback((companyId) => {
    setMenuOpen(prev => prev === companyId ? null : companyId);
  }, []);
